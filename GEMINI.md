...
This file contains context and guidelines for AI assistants (including Gemini) when working with code in this repository.
...
**linter_test.go improvements:**
- Consolidated duplicate test runner functions into `runTestWithSettings`
- Extracted constants for setting names (`settingMaxLineLen`, etc.)
- Broke down monolithic `updateGolden` into smaller functions
- Standardized all comments to English
- Removed duplicate `fileEdit` type definition
- Improved error handling in golden file updates
- Used constants consistently (e.g., `goldenFilePerms` instead of hardcoded `0644`)

**Diagnostic message improvements:**
- Changed `"Signature fits in one line"` → `"Multi-line signature can be collapsed to one line"` (more actionable)
- Changed `"Signature can be formatted better"` → `"Signature can be reformatted more compactly"` (removed subjective "better")
- Updated all test files and golden files to match new messages

### Build System

**Makefile added** with targets for:
...
- `make build-example` - Build example binary
- `make run-example` - Run linter on example
...
1. **Testing**: Always run tests after making changes. Use `make test` or `go test ./...`
2. **Golden files**: If changing diagnostic messages, update both test files and golden files
3. **Code style**: Maintain English comments, use constants for strings, break down large functions
4. **Performance**: Keep the linter fast - avoid type loading, minimize allocations
5. **Configuration**: Remember all 4 settings when discussing configuration
6. **Examples**: Provide concrete before/after code examples when explaining behavior
